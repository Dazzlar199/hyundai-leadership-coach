...
// 1. Updated Regex to be more robust
const valueTagRegex = /#([^\s#.,!?;:]+)/g;

// 2. Component to parse and render the script
function ScriptRenderer({ script }: { script: string }) {
  const { dialogueParts, summary } = useMemo(() => {
    ...
    if (summaryIndex !== -1) {
      ...
      summaryScript = cleanedScript.substring(summaryIndex + summaryMarker.length).trim();
    }
    
    const dialogueLines = dialogueScript.split('\n').filter(line => line.trim().startsWith('**'));
  ...
  return (
    <div className="space-y-4">
      {dialogueParts.map((part, index) => {
        ...
        return (
          <div key={index} className={`flex gap-3 ${isLeader ? '' : 'flex-row-reverse'}`}>
            <div className={`p-2 rounded-full h-fit ${isLeader ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                {getIcon(part.speaker)}
            </div>
            <div className={`w-full p-4 rounded-lg ${isLeader ? 'bg-primary/10' : 'bg-muted'}`}>
              <p className="font-semibold text-sm mb-1">{part.speaker}</p>
...
export function ScenarioPreview({ scenario, onReset }: ScenarioPreviewProps) {
  ...
  const startAssessment = useScenarioStore((state) => state.startAssessmentWithScenario);

  const handleStartAssessment = () => {
    const cleanText = scenario.scriptMd.replace(/\*\*.*?\:\*\*/g, '').replace(/#([^\s#.,!?;:]+)/g, '').trim();
    ...
    <div className="grid gap-8 lg:grid-cols-3">
      ...
      <div className="space-y-8">
        <Card>
          ...
          <CardContent>
            {uniqueTags.length === 0 ? (
              <p className="text-sm text-muted-foreground">태그를 찾을 수 없습니다.</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {uniqueTags.map(tag => <Badge key={tag} variant="secondary">{tag}</Badge>)}
              </div>
            )}
          </CardContent>
        </Card>
        <div className="text-center space-y-2">
          <Button onClick={handleStartAssessment} className="w-full">이 시나리오로 진단하기</Button>
          <Button onClick={onReset} className="w-full" variant="outline">새로운 시나리오 만들기</Button>
        </div>
      </div>
    </div>
  );
